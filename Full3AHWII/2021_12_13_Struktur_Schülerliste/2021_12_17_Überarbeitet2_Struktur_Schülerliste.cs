...
namespace _20211213_Struktur_Schülerliste
{
    ...
    {
        //Function "Eingabe"
        static Schueler Eingabe(int KatNr, string Vorname, string Nachname, int Alter)
        {
            //Create an pseudo Schueler for return
            Schueler schueler;

            //Input the data
            schueler.KatNr = KatNr;
            schueler.Vorname = Vorname;
            schueler.Nachname = Nachname;
            schueler.Alter = Alter;
        ...
        //Function "MaxAlter"
        static List<Schueler> MaxAlter(Schueler[] Klasse)
        {
            //Create a list to safe the MaxAlter
            List<Schueler> MaxAlter = new List<Schueler>();
        ...
        static void Main(string[] args)
        {
            ...
            int KatNr = Convert.ToInt32(Console.ReadLine());
            ...
            Schueler schueler1 = Eingabe(KatNr, Vorname, Nachname, Alter);
            ...
            for(int i = 0; i < Schulklasse.Length; i++)
            {
                ...
                Console.Write("Geben Sie bitte die KatNr ein: ");
                KatNr = Convert.ToInt32(Console.ReadLine());
                Console.Write("Geben Sie bitte den Vornamen ein: ");
                ...
                Schulklasse[i] = Eingabe(KatNr, Vorname, Nachname, Alter);
            }
            ...
            //MaxAlter return
            List<Schueler> schueler_max_Alt = MaxAlter(Schulklasse);

            //Output all values from the oldest schueler
            Console.WriteLine("Der/Die älteste/n Schüler: ");
            for(int i = 0; i < schueler_max_Alt.Count; i++)
            {
                Anzeigen(schueler_max_Alt[i]);
                Console.WriteLine(" ");
            }
        }
    }
